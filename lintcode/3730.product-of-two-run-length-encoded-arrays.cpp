...
public:
     ...
     */
    vector<vector<int>> findRunLengthArray(vector<vector<int>> &encoded1, vector<vector<int>> &encoded2) {
        ...
        while (i < n && j < m) {
            int tmp = encoded1[i][0] * encoded2[j][0];
            if (res.size() == 0 || res.back()[0] != tmp) {
                res.push_back({tmp, 1});
            } else {
                res.back()[1] += 1;
            }
