...
public:
    vector<string> spellchecker(vector<string>& wordlist, vector<string>& queries) {
        ...
        for (auto &word: wordlist) {
            string lower = lowKey(word);
            string wildLower = vowelKey(word);
            if (cap.find(lower) == cap.end()) {
                cap[lower] = word;
            }

            if (vowel.find(wildLower) == vowel.end()) {
                vowel[wildLower] = word;
            }
        }
        ...
        for (auto &q: queries) {
            ...
            string lower = lowKey(q);
            if (cap.find(lower) != cap.end()) {
                res.push_back(cap[lower]);
                continue;
            }
            
            string wildLower = vowelKey(q);
            if (vowel.find(wildLower) != vowel.end()) {
                res.push_back(vowel[wildLower]);
            } else {
                res.push_back("");
            }
        }
